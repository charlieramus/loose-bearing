...
export class AppController {
  ...
  private onMapClick(coord: LatLng): void {
    ...
    if (outcome.ok) this.acceptEndpoint(outcome.endpoint, true);
    else {
      // Clicked genuinely off the network — show it on that field, no pin.
      this.pins.clear(role);
  ...
  private renderRouted(
    ...
    // The render is a pure function of the router result (RenderPlan) — V5 reuses it.
